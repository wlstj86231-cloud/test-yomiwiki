document.addEventListener('DOMContentLoaded', () => {
    ...
    window.encodeWikiTitle = (t) => encodeURIComponent(window.titleToSlug(t)).replace(/[!'()*]/g, c => `%${c.charCodeAt(0).toString(16).toUpperCase()}`);
    ...
    if (si && sd) { let dt; si.addEventListener('input', () => { clearTimeout(dt); const q = si.value.trim(); if (q.length < 1) { sd.style.display = 'none'; return; } dt = setTimeout(async () => { try { const r = await fetch(`${API_ENDPOINT}/search/suggest?q=${encodeURIComponent(q)}`); const s = await r.json(); if (s.length > 0) { sd.innerHTML = s.map(i => `<div class="dropdown-item" onclick="window.navigateTo('/w/${window.encodeWikiTitle(i)}')">${escapeHTML(i)}</div>`).join(''); sd.style.display = 'block'; } else sd.style.display = 'none'; } catch (e) { } }, 1); }); document.getElementById('search-btn').onclick = () => { const q = si.value.trim(); if (q) window.navigateTo(`/w/${window.encodeWikiTitle(q)}`); }; si.onkeydown = (e) => { if (e.key === 'Enter') document.getElementById('search-btn').onclick(); }; }
    ...
    async function renderArticle(t) {
        ...
        try {
            ...
            const ib = isTopLevelBoardTitle(d.title); const ih = d.is_hub || d.title === 'SubSector_Archive';
            ...
            const ia = currentUser?.role === 'admin'; const eb = (d.title === 'Main_Page' ? ia : (ia || currentUser?.username === d.author)) ? `<a href="/w/${window.encodeWikiTitle(d.title)}?mode=edit" class="btn-clinical-toggle" style="font-size:0.65rem; margin-left:5px;">[EDIT_NODE]</a>` : ""; const hb = `<a href="/w/${window.encodeWikiTitle(d.title)}?mode=history" class="btn-clinical-toggle" style="font-size:0.65rem; margin-left:5px;">[HISTORY]</a>`;
            ...
            if (ib || ih) {
                ...
                <table class="clinical-table" style="width:100%; border-collapse:collapse; font-size:0.8rem;">
                    ...
                    <tbody>${sl.map(x => {
                        ...
                        return `<tr onclick="window.navigateTo('/w/${window.encodeWikiTitle(x.title)}')" style="border-bottom:1px solid #111; cursor:pointer; background:${r};">
    ...
    window.establishNewNode = (s, n = false) => { const m = n ? "edit&type=notice" : "edit"; const nm = prompt(n ? "NOTICE Title:" : "Node Title:"); if (nm) window.navigateTo(`/w/${window.encodeWikiTitle(s + "/" + nm)}?mode=${m}`); };
    ...
    async function renderHistory(t) {
        ...
        try {
            ...
            mt.innerHTML = `<a href="/w/${window.encodeWikiTitle(t)}" class="btn-clinical-toggle" style="font-size:0.65rem;">[BACK]</a>`;
            ...
            ab.innerHTML = `<table class="clinical-table" style="width:100%; border-collapse:collapse; font-size:0.8rem;">
                ...
                <tbody>${revs.map((rev, i) => `<tr style="border-bottom:1px solid #111; cursor:pointer;" onclick="window.navigateTo('/w/${window.encodeWikiTitle(t)}?rev=${parseInt(rev.id)}')">
    ...
    window.adminPurgeArticle = async (t) => {
        ...
        try {
            ...
            if (r.ok) {
                ...
                const targetPath = `/w/${window.encodeWikiTitle(t)}`;
                if (currentPath === targetPath) {
                    const lastSlash = t.lastIndexOf('/');
                    const parent = lastSlash !== -1 ? t.substring(0, lastSlash) : "Main_Page";
                    window.navigateTo(`/w/${window.encodeWikiTitle(parent)}`);
                } else {
                    init();
                }
            } else {
