const knowledge = {
  ...
  '양파 저장 온도': {
    ...
    link:'https://boribay.com/guides/garak-onion-price-lookup?utm_source=yomiwiki.com&utm_medium=owned_referral&utm_campaign=knowledge_desk&utm_content=onion_storage',
    linkText:'양파 품종·규격별 참고가격 확인하기'
  },
  '중고 트랙터 사용시간': {
...
function render(query,track=false){
  ...
  const contextPanel=contextLink.closest('.answer-next');
  contextPanel.hidden=!data.link;
  if(data.link){contextLink.href=data.link;contextLink.innerHTML=`${data.linkText} <i data-lucide="arrow-right" aria-hidden="true"></i>`;}
  ...
  nextPanel.hidden=!data.article;
  if(data.article){const nextLink=document.querySelector('#next-article-link');nextLink.href=`/knowledge/${data.article}`;nextLink.innerHTML=`${data.title} 자세히 읽기 <i data-lucide="arrow-right" aria-hidden="true"></i>`;}
